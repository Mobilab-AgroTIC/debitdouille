...
// Flowmeter
const byte debitmetreG = 32; // broche utilisée pour déclencher l'interruption du debitmètre 1
const byte debitmetreD = 33; // broche utilisée pour déclencher l'interruption du debitmètre 1
volatile long nbPulseG = 0;  // compteur d'impulsions débitmètre 1
volatile long nbPulseD = 0;  // compteur d'impulsions débitmètre 2
byte pulsesG;
byte pulsesD;
long NombreDimpulsionsG = 0;
long NombreDimpulsionsD = 0;
float debitG;
float debitD;
unsigned long timer;
int NbImpulsionsDebitmetreG = 1000;  //Pulses debitmetre gauche
int NbImpulsionsDebitmetreD = 1000;  //Pulses debitmetre droit
int correctionManometreA = 70;  //Correction manometreA (coeff)
int correctionManometreB = 70;  //Correction manometre (constante)

unsigned int constDebG;
unsigned int constDebD;
...
void ICACHE_RAM_ATTR comptageG() {  // incremente nbPulse1 à chaque interruption
  nbPulseG++;
}
void ICACHE_RAM_ATTR comptageD() {  // incremente nbPulse1 à chaque interruption
  nbPulseD++;
}
...
void setup() {
  ...
  pinMode(debitmetreG, INPUT_PULLUP);
  pinMode(debitmetreD, INPUT_PULLUP);
  // Mettre la broche du débimetre en interrupt, assignation de la fonction comptage et réglage en RISING
  attachInterrupt(digitalPinToInterrupt(debitmetreG), comptageG, RISING);  // a chaque interruption lance comptageG
  attachInterrupt(digitalPinToInterrupt(debitmetreD), comptageD, RISING);  // a chaque interruption lance comptageD

  //Récupération des constantes et stockage
  preferences.begin("constantes", false);
  constDebG = preferences.getUInt("constDebG", 0);
  constDebD = preferences.getUInt("constDebD", 0);
  constManA = preferences.getUInt("constManA", 0);
  constManB = preferences.getUInt("constManB", 0);

  preferences.end();
  if (constDebG != 0) {
    NbImpulsionsDebitmetreG = constDebG;
  }
  if (constDebD != 0) {
    NbImpulsionsDebitmetreD = constDebD;
  }
  if (constManA != 0) {
...
if(currentMillis - startMillis >= period) // Calcule et envoie toutes les 1 secondes environ
  {
    ...
    // L/min
    debitG = 60000.0 / (currentMillis - startMillis) * pulsesG / NbImpulsionsDebitmetreG;
    debitD = 60000.0 / (currentMillis - startMillis) * pulsesD / NbImpulsionsDebitmetreD;
...
//      Serial.println(gps.altitude.meters(), 0);
    ...
    bluetoothMsg = "A;" + String(pressure) + ";" + String(sat) + ";" + String(lon) + ";" + String(llat) + ";" + String(sspeed) + ";" + String(debitG) + ";" + String(debitD);
  ...
  if (messageRecu == "cns") {
    ...
    bluetoothMsg = "B;" + String(NbImpulsionsDebitmetreG) + ";" + String(NbImpulsionsDebitmetreD) + ";" + String(correctionManometreA) + ";" + String(correctionManometreB);
    SerialBT.println(bluetoothMsg);
    Serial.println(bluetoothMsg);
//    Serial.println(messageRecu);
    messageRecu = "";
//    Serial.println(messageRecu);
  ...
  if (messageRecu == "gauche") {
    ...
    preferences.begin("constantes", false);
    preferences.putUInt("constDebG", valeurRecue);
    preferences.end();
    messageRecu = "cns";
  }
  if (messageRecu == "droit") {
    ...
    //Ecriture de la constante gauche
    preferences.begin("constantes", false);
    preferences.putUInt("constDebD", valeurRecue);
    preferences.end();
    messageRecu = "cns";
  }
